 ...
 * @LastEditTime: 2021-08-02 10:34:11
...
export default {
  name: 'IncomingRetrungoodsDetails',
  mixins: [search],
  data () {
    return {
      loading: false,
  ...
  methods: {
    ...
    getVarietiesDateilList (type) {
      ...
      const data = {
        docNo: this.docNo,
        expendContractId: this.expendContractId,
        incomingId: this.incomingId,
        projectId: this.projectId,
        queryParam: this.materialName,
        sourceType: this.sourceType
      };
      this.$store.dispatch('warIncoming/getIncomingRetrungoodsDetails', data).then(res => {
        if (res.status === 0) {
          const selectList = [];
          const selectIds = [];
          for (const item of this.cloneSelectList) {
            selectIds.push(`${item.hashColumn}-${item.incomingDetailId}`);
          }
          this.selectId = selectIds; // 默认勾选
          res.results[0].incomingDetails.forEach(item => {
            item.incomingDetailId = item.id;
            item.warehouseId = res.results[0].warehouseId;
            item.rowKey = `${item.hashColumn}-${item.incomingDetailId}`;
            const index = this.cloneSelectList.findIndex(v => `${v.hashColumn}-${v.incomingDetailId}` === item.rowKey);
            if (index >= 0) {
          ...
          if (this.active === 1) {
            this.cloneSelectList.forEach(sele => {
              const key = selectList.findIndex(v => `${v.hashColumn}-${v.incomingDetailId}` ===
                `${sele.hashColumn}-${sele.incomingDetailId}`);
              if (key < 0) {
                this.temTableData.push(sele);
              }
            });
          }
          this.mainTable.tableData = res.results[0].incomingDetails;
