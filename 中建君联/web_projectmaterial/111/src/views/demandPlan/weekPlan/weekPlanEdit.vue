<!--
 * @Author: your name
 * @Date: 2020-07-24 15:24:48
 * @LastEditTime: 2023-02-15 14:44:17
 * @LastEditors: wumaoxia 1805428335@qq.com
 * @Description: In User Settings Edit
 * @FilePath: \web_projectmaterial\src\views\demandPlan\weekPlan\weekPlanEdit.vue
-->
<template>
  <div>
      ...
      <div slot="editPage">
          ...
          <template slot="form-projectUnitId" slot-scope="{scope, item}">
            <el-form-item :label="$t(item.label)" :prop="item.prop" :class="item.label.length > 7 && 'maxLabel'"
              ...
              <el-select v-model="scope[item.prop]"
                ...
                @focus="handleFocus(item, 'focus')" @change="chackProject(undefined,undefined,scope, item)">
        ...
        <div v-if="pageConfigLoading">
          <div v-for="subTable in pageConfig.subTableConfig" :key="subTable.subTableName">
            <g-edit-title :title="$t(`weekPlan.${subTable.subTableName}`)"
              :titleTips="$t(`weekPlan.${subTable.titleTips}`)">
    ...
    <g-dialog v-dialogDrag v-if="dialogVisible" :dialogConfig="dialogConfig" :isVisible.sync="dialogVisible">
      <innerOuterPlanDetials slot="body" slot-scope="{height}" :dialogHeight="height" :close.sync="dialogVisible"
        ...
        varietiesTitle="weekPlan.masterPlan" @getData="getData" v-if="dialogVisible">
...
export default {
  ...
  data () {
    const currentApplyQuantityValid = (rule, value, callback) => {
      if (!value) {
        callback(new Error(this.$t('weekPlan.currentApplyQuantityTips')));
      } else {
        callback();
      }
    };
    return {
      ...
      dialogVisible: false,
      dialogConfig: {
        title: this.$t('weekPlan.seleMaterialVarieties'),
        appendBody: false,
        center: true,
        top: '50px',
        width: '95%',
  ...
  methods: {
    ...
    async _getInfoData (id = 0) {
      await this.handleGetInfoData(id, this.page.PageConfig.processParmas.infoUrl, () => {
        ...
        this.getUnitList();
        this.displayFieldsAccordingToProjectControlType('demandWeekPlanDetail');
    ...
    handleSetRequiredDateUsePlace () {
      ...
      const tableData = this.pageConfig.subTableConfig['demandWeekPlanDetail'].tableData;
    ...
    _getProjectInfo () {
      ...
      this.$store.dispatch('weekPlan/getProjectInfo', data).then(res => {
        if (res.results && res.results.length) {
          ...
          this.back = this.$t('weekPlan.backTips');
          this.$set(this.pageConfig.projectForm, 'partyAUnitId', '');
          this.$set(this.pageConfig.projectForm, 'partyAUnitName', '');
          this.$set(this.pageConfig.projectForm, 'projectUnitName', '');
          this.$set(this.pageConfig.projectForm, 'projectUnitId', '');
    ...
    getProjectMajorlist () {
      ...
      this.$store.dispatch('projectMajor/getProjectMajorlist', data).then(res => {
        if (res.status === 0) {
          const tableList = this.pageConfig.subTableConfig['demandWeekPlanDetail'].tableList.slaveColumns;
    ...
    sysHandleDeletaAdd (tableName) {
      ...
      if (!this.pageConfig.projectForm.applyDate) {
        // 请选择填报周期
        MsgList.push(this.$t('weekPlan.applyWeek'));
      }
    ...
    sysHandleSelectDetail (tableName) {
      ...
      // 去掉未添加需求总计划判断
      // if (this.back) {
      //    MsgList.push(this.$t('weekPlan.backTips'));
      // }
      if (MsgList && MsgList.length) {
        return this.$message.error(`${this.$t('tips.pleaseSelect')}${MsgList.join(', ')}`);
      }
    ...
    // 校验项目是否已做周期
    chackProject (tableName, callback, row, item) {
      this.row = row;
      this.item = item;
      ...
      this.$store.dispatch('weekPlan/checkWeekplan', data).then(res => {
        if (res.results) {
          // 该项目已添加此周需求计划，不能再次添加。
          this.$message.error(this.$t('weekPlan.checkWeekplanTips'));
    ...
    // 校验项目是否已做周期
    // chackProject (row, item, tableName, callback) {
    //   this.week = this.getWeekInMonth(new Date(this.pageConfig.projectForm.applyDate));
    //   const data = {
    //     weekPlanId: this.id || null,
    //     projectId: this.pageConfig.projectForm.projectId,
    ...
    getData (arr) {
      ...
      const tableDataList = this.pageConfig.subTableConfig['demandWeekPlanDetail'].tableData;
      const tableData = tableDataList.concat(data);
      this.$set(this.pageConfig.subTableConfig['demandWeekPlanDetail'], 'tableData', tableData);
      this.dialogVisible = false;
      !this.pageConfig.projectForm.mainDemandDetail && this.setPurchaseContent('demandWeekPlanDetail');
    },
    // 批量删除后计算
    ...
    handleSave (isProcess = false) {
      ...
      this.$refs.editForm.getValidateForm(() => {
        ...
        const demandWeekPlanDetail = this.$refs.demandWeekPlanDetail[0].validateTableData();
        if (!demandWeekPlanDetail || demandWeekPlanDetail.length === 0) {
          // 存在未填写或个数错误的数据
          this.$message.error(`${this.$t('weekPlan.demandWeekPlanDetail')}${this.$t('tips.notFilledIn')}`);
          return;
        }
        // 判断明细是否重复
        // const selectTableList = [];
        // for (const i in demandWeekPlanDetail) {
        //   const item = demandWeekPlanDetail[i];
        ...
        for (const i in demandWeekPlanDetail) {
          ...
          if (quantityList && quantityList.length) {
              ...
              this.$message.error(`${this.$t('tips.theFirst')}${sorts}${this.$t('tips.row')}：${this.$t('tips.pleaseEnter')}${this.$t('weekPlan.currentApplyQuantity')}`);
              return;
          }
        ...
        data.weekPlanDetails = demandWeekPlanDetail;
    ...
    sysHandleDownloadTemplate () {
      ...
      const tips = this.$t('weekPlan.downTips');
    ...
    handleDownloadTemplate() {
      this.$store.dispatch('monthPlan/getDownloadTemplate').then(data => {
        ...
        link.href = url;
        const fileName = 'menu.weekPlan';
        link.setAttribute('download', `${this.$t(fileName)}${this.$t('weekPlan.demandWeekPlanDetail')}${this.$t('fConfig.template')}.xls`);
    ...
    canImprotDetail () {
      ...
      if (this.pageConfig.subTableConfig.demandWeekPlanDetail.tableData.length !== 0) {
