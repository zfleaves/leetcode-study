                  ...
                  <div class="custom-tree-node" slot-scope="{node, data}" :class="'level' + data.level">
                    <div class="tree-node" v-if="node.level === 1">
                      <div class="varietiesName" style="width: 100%;">
                        <tooltips :value="`${$t('tips.theFirst')}${data.schemeNum || 1}${$t('dialog.schemeNumTips')}-${data.docNo}`">
                        </tooltips>
                      </div>
                    </div>
...
export default {
  ...
  async created () {
    ...
    if (this.planType === 'year') {
        this.varietiesDateilUrl = 'yearPlan/getDemandYearList';
    }
    if (this.planType === 'quarter') {
        this.varietiesDateilUrl = 'quarterPlan/getDemandQuarterList';
    }
    const selectList = this.$clone(this.selectList);
    selectList.forEach((item, index) => {
      const rowKey = item.eId;
  ...
  methods: {
        ...
        if (res.status === 0) {
          ...
          for (const item of results) {
            ...
            for (const children of item.children) {
                ...
                this.$set(children, 'rowKey', children.id);
    ...
    handleCheckChange () {
      ...
      const data = selectVarietiesList.concat(this.checkList);
      this.selectMainTable.tableData = data;
    },
    // 点击行
