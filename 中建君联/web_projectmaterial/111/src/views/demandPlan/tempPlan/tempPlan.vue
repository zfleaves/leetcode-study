 ...
 * @LastEditTime: 2020-07-31 15:36:25
 * @LastEditors: Please set LastEditors
 * @Description: In User Settings Edit
 * @FilePath: \web_projectmaterial\src\views\demandPlan\tempPlan\tempPlan.vue
-->
<template>
  <div>
    <query-page
    ...
    <g-dialog v-if="dialogVisible" :dialogConfig="dialogVisibleConfig" :isVisible.sync="dialogVisible">
      <div class="content" slot="body">
        <div class="title">
          <!-- 下单方式 -->
          <span class="title-tips">{{$t('tempPlan.orderingMethod')}}：</span>
          <span>
              <!--下送货通知单、下零星采购单。  -->
              <el-radio v-model="fastSupplyOrder" label="supplyOrder">{{$t('tempPlan.delivery')}}</el-radio>
              <el-radio v-model="fastSupplyOrder" label="sporadicPurchase">{{$t('tempPlan.sporadicPurchase')}}</el-radio>
          </span>
        </div>
        <!-- 注：下送货通知单时必须关联一个合同进行下单。 -->
        <div class="tipsText">
          <span style="color: red;">{{$t('tempPlan.explainTips')}}</span>
        </div>
      </div>
  ...
  export default {
    ...
    data () {
      return {
        ...
        dialogVisible: false,
        dialogVisibleConfig: {
          title: this.$t('tempPlan.quickOrder'),
          appendBody: false,
          center: true,
          top: '200px',
          width: '25%',
    ...
    methods: {
      // 获取表单
      _getTableDataList () {
        this.handleGetTableDataList('tempPlan/getPageList');
      },
      // 删除
      handleDelete(row) {
        const statusConfig = {
      ...
      drawerItemSave() {
        ...
        if (!this.fastSupplyOrder) {
          return this.$message.error(`${this.$t('tips.pleaseSelect')}${this.$t('tempPlan.orderingMethod')}`);
        }
      ...
      handleQuickOrderRouter(row, name, type = 'order') {
        ...
        const orderSource = this.$base64.encode('tempPlan');
