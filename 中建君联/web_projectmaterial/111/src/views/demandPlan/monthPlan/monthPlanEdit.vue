<!--
 * @Author: your name
 * @Date: 2020-07-24 15:21:19
 * @LastEditTime: 2023-02-15 14:46:01
 * @LastEditors: wumaoxia 1805428335@qq.com
 * @Description: In User Settings Edit
 * @FilePath: \web_projectmaterial\src\views\demandPlan\monthPlan\monthPlanEdit.vue
-->
<template>
  <div>
    <edit-page @editEvent="editEvent" v-loading="loading">
      <div slot="editPage">
        <g-edit-form ref="editForm" v-if="pageConfigLoading" :type="type" :tableConfig="pageConfig.mainFormConfig"
          ...
          <!-- 申报料单位 -->
          <template slot="form-projectUnitId" slot-scope="{scope, item}">
              <el-form-item :label="$t(item.label)" :prop="item.prop" :class="item.label.length > 7 && 'maxLabel'"
              :title="$t(item.label)">
              <el-select v-model="scope[item.prop]"
                ...
                @focus="handleFocus(item, 'focus')" @change="projectUnitIdFun(scope, item)">
        ...
        <div v-if="pageConfigLoading">
          <div v-for="subTable in pageConfig.subTableConfig" :key="subTable.subTableName">
            <g-edit-title :title="$t(`monthPlan.${subTable.subTableName}`)"
              :titleTips="$t(`monthPlan.${subTable.titleTips}`)">
              <g-button :operationButtons="subTable.subTableButton" @editTableBut="mainOperateBtnSubTable">
                ...
                <template v-if="sysHandleDeletaAddShow" slot-scope="{parameter}" slot='button-sysHandleDeletaAdd'>
                  <el-button size="small" type="primary" :plain="false" :disabled="parameter.disabled"
                    icon="el-icon-plus" v-fast-click @click="sysHandleDeletaAdd(parameter.subTableCode)">
                    {{$t('button.addDetails')}}
                  </el-button>
                </template>
                <!-- 导入 -->
            ...
            <g-edit-pl-table :ref="subTable.subTableName" :type="type" :isSelection="subTable.isSelection"
              ...
              <template slot="edit-table-executeVarietiesName" slot-scope="{item, scope}">
                <el-input :class="changeStyle(scope, item.prop, 'varietiesName')" size="small"
                  ...
                  <el-tooltip v-if="scope.varietiesId && scope[item.prop] !== scope.varietiesName" slot="suffix"
                    class="item" effect="dark" :content="`${$t('tips.primary')}：${scope.varietiesName}`"
                    placement="top">
                    <!-- 变 -->
              ...
              <template slot="edit-table-executeStandards" slot-scope="{item, scope}">
                <el-input :class="changeStyle(scope, item.prop, 'standards')" size="small"
                  :disabled="$route.name === 'processApprovalPage' ? item.inputStatus === 'disable' : !scope.varietiesId ? false : item.inputStatus === 'disable'"
                  :maxlength="Number(item.maxlength)" :clearable="item.clearable === undefined ? true : item.clearable"
                  v-model.trim="scope[item.prop]" :placeholder="item.placeholder">
                  <el-tooltip v-if="scope.varietiesId && scope[item.prop] !== scope.standards" slot="suffix"
                    class="item" effect="dark" :content="`${$t('tips.primary')}：${scope.standards}`" placement="top">
                    <!-- 变 -->
                    <!-- <span class="changeStatus">{{$t('tips.changeTips')}}</span> -->
                    <span class="changeStatus"><i class="el-icon-view smallFont error" style="right: 5px;"></i></span>
                  </el-tooltip>
                </el-input>
              </template>
              ...
              <template slot="edit-table-executeUnit" slot-scope="{item, scope}">
                <el-input :class="changeStyle(scope, item.prop, 'unit')" size="small"
                  :disabled="$route.name === 'processApprovalPage' ? item.inputStatus === 'disable' : !scope.varietiesId ? false : item.inputStatus === 'disable'"
                  :maxlength="Number(item.maxlength)" :clearable="item.clearable === undefined ? true : item.clearable"
                  v-model.trim="scope[item.prop]" :placeholder="item.placeholder">
                  <el-tooltip v-if="scope.varietiesId && scope[item.prop] !== scope.unit" slot="suffix" class="item"
                    effect="dark" :content="`${$t('tips.primary')}：${scope.unit}`" placement="top">
                    <!-- 变 -->
                    <span class="changeStatus"><i class="el-icon-view smallFont error" style="right: 5px;"></i></span>
                  </el-tooltip>
                </el-input>
              ...
              <template slot="edit-table-quantitiesName" slot-scope="{item, scope, rowIndex}">
                <div class="slot-input"
                  v-if='pageConfig.projectForm.costControlRuleCode === "01" || pageConfig.projectForm.costControlRuleCode === "03"'>
                  <el-input v-if="!scope[item.prop]" v-model.trim="scope[item.prop]" :size="item.size || 'small'"
                    readonly>
                    <template slot="suffix" v-if="item.inputStatus !== 'disable' && pageDisabled">
                      <i v-if="scope[item.prop]" class="slot-close el-icon-circle-close"
              ...
              <template slot="edit-table-majorCode" slot-scope="{item, scope, rowIndex}">
                <el-select v-if="item.inputStatus !== 'disable' && pageDisabled && scope.demandType === '02' ||
                 (item.inputStatus !== 'disable' && pageDisabled && scope.demandType === '01' && !scope[item.prop])"
                  v-model.trim="scope[item.prop]" :placeholder="item.placeholder" :size="item.size || 'small'"
                  :disabled="item.inputStatus === 'disable'" :filterable="item.filterable || true" style="width:100%"
                  :clearable="item.clearable === undefined ? true : item.clearable"
                  @change="handleChange(item, scope, $event, rowIndex, subTable)">
    ...
    <g-dialog v-dialogDrag v-if="dialogVisible" :dialogConfig="dialogConfig" :isVisible.sync="dialogVisible">
      <innerOuterPlanDetials slot="body" slot-scope="{height}" :dialogHeight="height" :close.sync="dialogVisible"
        ...
        varietiesTitle="monthPlan.masterPlan" @getData="getData" v-if="dialogVisible">
    ...
    <g-dialog v-if="projectChildFlag" :dialogConfig="projectChildDialogConfig" :isVisible.sync="projectChildFlag">
      <usePlaceDialog slot="body" slot-scope="{height}" :dialogHeight="height"
        ...
        :projectId="pageConfig.projectForm.projectId" :close.sync="projectChildFlag" @getData="getProjectChild"
        v-if="projectChildFlag">
      </usePlaceDialog>
    </g-dialog>
    <!-- 导入 -->
        ...
        <g-edit-pl-table slot="body" ref="importTable" :height='height' :type="type"
          ...
          <template slot="edit-table-executeVarietiesName" slot-scope="{item, scope}">
            <el-input :class="changeStyle(scope, item.prop, 'varietiesName')" size="small"
              ...
              v-model.trim="scope[item.prop]" :placeholder="item.placeholder">
              <el-tooltip v-if="scope.varietiesId && scope[item.prop] !== scope.varietiesName" slot="suffix"
                class="item" effect="dark" :content="`${$t('tips.primary')}：${scope.varietiesName}`" placement="top">
                <!-- 变 -->
                <span class="changeStatus"><i class="el-icon-view smallFont error" style="right: 5px;"></i></span>
              </el-tooltip>
            </el-input>
          ...
          <template slot="edit-table-executeStandards" slot-scope="{item, scope}">
            <el-input :class="changeStyle(scope, item.prop, 'standards')" size="small"
              ...
              :maxlength="Number(item.maxlength)" :clearable="item.clearable === undefined ? true : item.clearable"
              v-model.trim="scope[item.prop]" :placeholder="item.placeholder">
              <el-tooltip v-if="scope.varietiesId && scope[item.prop] !== scope.standards" slot="suffix" class="item"
                effect="dark" :content="`${$t('tips.primary')}：${scope.standards}`" placement="top">
                <!-- 变 -->
                <span class="changeStatus"><i class="el-icon-view smallFont error" style="right: 5px;"></i></span>
              </el-tooltip>
            </el-input>
...
export default {
  ...
  data () {
    const currentApplyQuantityValid = (rule, value, callback) => {
      if (!value) {
        callback(new Error(`${this.$t('tips.pleaseEnter')}${this.$t('monthPlan.currentApplyQuantity')}`));
      } else {
        callback();
      }
    };
    return {
      ...
      dialogVisible: false,
      dialogConfig: {
        title: this.$t('monthPlan.seleMaterialVarieties'),
        appendBody: false,
        center: true,
        top: '50px',
        width: '95%',
  ...
  methods: {
    ...
    async _getInfoData (id = 0) {
      await this.handleGetInfoData(id, this.page.PageConfig.processParmas.infoUrl, () => {
        this.projectUnitId = this.$clone(this.pageConfig.projectForm.projectUnitId);
        this.projectUnitName = this.$clone(this.pageConfig.projectForm);
        // 获取专业
        this.getProjectMajorlist();
        this.getUnitList();
        this.displayFieldsAccordingToProjectControlType('demandMonthPlanDetail');
      });
    },
    // 获取校验规则
    _mastercheckList () {
    ...
    handleSetRequiredDateUsePlace () {
      ...
      const tableData = this.pageConfig.subTableConfig['demandMonthPlanDetail'].tableData;
    ...
    _getProjectInfo () {
      ...
      this.$store.dispatch('monthPlan/getProjectInfo', data).then(res => {
        if (res.results && res.results.length) {
          ...
          // 未添加需求总计划
          this.$message.error(this.$t('monthPlan.backTips'));
          this.back = this.$t('monthPlan.backTips');
          this.$set(this.pageConfig.projectForm, 'partyAUnitId', '');
          this.$set(this.pageConfig.projectForm, 'partyAUnitName', '');
          this.$set(this.pageConfig.projectForm, 'projectUnitName', '');
          this.$set(this.pageConfig.projectForm, 'projectUnitId', '');
    ...
    getProjectMajorlist () {
      ...
      this.$store.dispatch('projectMajor/getProjectMajorlist', data).then(res => {
        if (res.status === 0) {
          const tableList = this.pageConfig.subTableConfig['demandMonthPlanDetail'].tableList.slaveColumns;
    ...
    sysHandleDeletaAdd (tableName) {
      ...
      if (!this.pageConfig.projectForm.projectName) {
        // this.$message.error(this.$t('tips.pleaseSelectProject'));
        // return;
        MsgList.push(this.$t('fConfig.projectName'));
      }
      if (!this.pageConfig.projectForm.applyMonth) {
        // 请选择填报月份
        // this.$message.error(this.$t('monthPlan.pleaseSelectApplyMonth'));
        // return;
        MsgList.push(this.$t('monthPlan.applyMonth'));
      }
      if (MsgList && MsgList.length) {
        return this.$message.error(`${this.$t('tips.pleaseSelect')}${MsgList.join(', ')}`);
      }
    ...
    // 校验项目是否已做周期
    chackProject (tableName, callback) {
      const data = {
        monthPlanId: this.id || null,
        projectId: this.pageConfig.projectForm.projectId,
        applyMonth: this.pageConfig.projectForm.applyMonth,
        projectUnitId: this.pageConfig.projectForm.projectUnitId
      };
      this.$store.dispatch('monthPlan/checkMonthplan', data).then(res => {
        if (res.results) {
          // 该项目已添加此月度需求计划，不能再次添加。
          this.$message.error(this.$t('monthPlan.checkMonthPlanTips'));
        } else {
          callback && callback();
        }
      });
    ...
    handleFocus() {
      this.projectUnitId = this.$clone(this.pageConfig.projectForm.projectUnitId);
      this.projectUnitName = this.$clone(this.pageConfig.projectForm.projectUnitName);
    },
    projectUnitIdFun(row, item) {
      const tableData = this.pageConfig.subTableConfig['demandMonthPlanDetail'].tableData;
      if (tableData && tableData.length) {
        const projectUnitName = this.pageConfig.projectForm.projectUnitName;
        const data = {
          monthPlanId: this.id || null,
          projectId: this.pageConfig.projectForm.projectId,
          applyMonth: this.pageConfig.projectForm.applyMonth,
        ...
        this.$store.dispatch('monthPlan/checkMonthplan', data).then(res => {
          if (res.results) {
            // 该项目已添加此月度需求计划，不能再次添加。
            this.$message.error(this.$t('monthPlan.checkMonthPlanTips'));
            this.$set(this.pageConfig.projectForm, 'projectUnitId', this.projectUnitId);
            this.$set(this.pageConfig.projectForm, 'projectUnitName', this.projectUnitName);
          } else {
            const index = item.selectList.findIndex(v => v.id === row.projectUnitId);
            if (index >= 0) {
              this.$set(this.pageConfig.projectForm, 'projectUnitName', item.selectList[index].name);
              this.$set(this.pageConfig.projectForm, 'projectUnitId', item.selectList[index].id);
              this.$set(this.pageConfig.projectForm, 'expendContractName', item.selectList[index].expendContractName);
              this.$set(this.pageConfig.projectForm, 'expendContractId', item.selectList[index].expendContractId);
              this.$set(this.pageConfig.projectForm, 'expendContractCode', item.selectList[index].expendContractCode);
              this.projectUnitId = item.selectList[index].id;
              this.projectUnitName = item.selectList[index].name;
    ...
    getData (arr) {
          ...
          data.push(
            {
              ...
              eId: item.demandType === '01' ? item.eId : null,
      ...
      const tableDataList = this.pageConfig.subTableConfig['demandMonthPlanDetail'].tableData;
      const tableData = tableDataList.concat(data);
      this.$set(this.pageConfig.subTableConfig['demandMonthPlanDetail'], 'tableData', tableData);
      this.dialogVisible = false;
      !this.pageConfig.projectForm.mainDemandDetail && this.setPurchaseContent('demandMonthPlanDetail');
    },
    // 批量删除后计算
    ...
    handleSave (isProcess = false) {
      ...
      this.$refs.editForm.getValidateForm(() => {
        ...
        const demandMonthPlanDetail = this.$refs.demandMonthPlanDetail[0].validateTableData();
        if (!demandMonthPlanDetail || demandMonthPlanDetail.length === 0) {
          // 存在未填写或个数错误的数据
          this.$message.error(`${this.$t('monthPlan.demandMonthPlanDetail')}${this.$t('tips.notFilledIn')}`);
          return;
        }
        // 是否存在重复明细
        // const selectTableList = [];
        // for (const i in demandMonthPlanDetail) {
        //   const item = demandMonthPlanDetail[i];
        ...
        for (const i in demandMonthPlanDetail) {
          ...
          // 既校验是否关闭只影响添加明细按钮和回写，与子表成本结构必填无关联。- 2022/11/07
          ...
          if (quantityList && quantityList.length) {
            ...
            this.$message.error(`${this.$t('tips.theFirst')}${sorts}${this.$t('tips.row')}：${this.$t('tips.pleaseEnter')}${this.$t('monthPlan.currentApplyQuantity')}`);
            return;
          }
        ...
        data.monthPlanDetails = demandMonthPlanDetail;
    ...
    sysHandleDownloadTemplate () {
      ...
      const tips = this.$t('monthPlan.downTips');
    ...
    handleDownloadTemplate () {
      this.$store.dispatch('monthPlan/getDownloadTemplate').then(data => {
        ...
        link.href = url;
        const fileName = 'menu.monthPlan';
        link.setAttribute('download', `${this.$t(fileName)}${this.$t('monthPlan.demandMonthPlanDetail')}${this.$t('fConfig.template')}.xls`);
    ...
    canImprotDetail (res) {
      ...
      if (MsgList && MsgList.length) {
        this.$message.error(`${this.$t('tips.pleaseSelect')}${MsgList.join(', ')}`);
        return false;
      }
      if (this.pageConfig.subTableConfig.demandMonthPlanDetail.tableData.length !== 0) {
