 ...
 * @LastEditTime: 2023-02-23 15:30:38
...
<template>
    <div>
        <edit-page @editEvent="editEvent" v-loading="loading" :isHasProcess="type !== 'renew'">
            <div slot="editPage">
                <g-edit-form
                    ref="editForm"
    ...
    methods: {
        ...
        getExpendContractData(arr) {
            if (arr && arr.length) {
                const parameter = {
                    settle: {
                        url: 'purchaseSettlement/getPageList',
                        data: {
                            contractId: arr[0].id
                        }
                    },
                    ...
                    pay: {
                        url: 'paymentApply/getPageList',
                        data: {
                            contractId: arr[0].id
                        }
                    }
                };
                this.getContractJudgeSettlePay(parameter, () => {
                    const params = {
        ...
        sysHandleDeletaBatch (tableName, otherOperate) {
            if (otherOperate) {
                ...
                if (this.deleteList[tableName] && this.deleteList[tableName].length) {
                    if (this.type === 'renew') {
                        const arr = this.deleteList[tableName].filter(item => !item.id);
                        if (!arr.length) {
                            this.$message.warning(this.$t('tips.cannotDelete'));
                            return;
                        }
                        this.handleDeleteSubTable(arr, this.$t(`${this.translateName}.${tableName}`), tableName);
                    } else {
        ...
        handleSave (isProcess = false) {
            this.$refs.editForm.getValidateForm(async () => {
                // if (this.isPayDataIncomplete) {
                //     // 当前合同已有付款申请正在进行中，请等待上一笔付款完成后再提交！
                //     this.$message.error(this.$t('tips.contractJudgePayTips2'));
                //     return;
                // }
                ...
                if (this.type === 'renew') {
                    purchasePaymentApplyDetail.forEach(item => this.$set(item, 'updateTime', this.$utils.commonUtil.formatTime(new Date())));
                }
        ...
        async checkAmount(data, purchasePaymentApplyDetail, isProcess) {
            ...
            data.paymentApplyDetails = purchasePaymentApplyDetail;
            // 项目资金设置
            this.getCapitalProjectPayment(data, tips, (tips) => {
                // 1
                ...
                if ((data.paymentCode !== '01' && data.paymentCode !== '03') && res.results.paymentControlRatio && (totalSettlementAmount * (paymentControlRatio / 100) < paymentAmount)) {
                    // 您的已结算额：{keyValue},
                    // 已付款额：{keyValue},
                ...
                // 4
                // 第4点质保金的判断调整：【如果合同质保金比例=0，则这个判断不触发。 如果质保金比例不等于0，则：申请付款的金额+已付款的金额 大于 结算总金额（1-质保金）时提示】
                const amount = Number((totalSettlementAmount * (1 - res.results.qualityDepositScale)).toFixed(2));
                if ((data.paymentCode !== '01' && data.paymentCode !== '03') && res.results.qualityDepositScale && (Number((applyPaymentAmount + totalPaymentAmount).toFixed(2)) > amount)) {
                ...
                if ((data.paymentCode !== '01' && data.paymentCode !== '03') && (Number((applyPaymentAmount + totalPaymentAmount).toFixed(2)) > totalInvoiceAmount)) {
        ...
        async getPaymentTotalamountContrast(data, tips, callback) {
            if (data.paymentCode === '01' || data.paymentCode === '03') {
        ...
        async getCapitalProjectPayment(data, tips, callback) {
            await this.$store.dispatch('publicApi/getCapitalProjectPaymentPublicApi', {projectId: data.projectId, costTypeCode: '03'}).then(res => {
                if (res.status === 0) {
                    ...
                    if (results.paymentControlCode === '03') {
                        ...
                        const totalLimitMaterialAmount = Number(results.totalLimitMaterialAmount) || 0; // 材料-项目资金限制支出金额
                        ...
                        if (Number((applyPaymentAmount + totalActualPaymentAmount).toFixed(2)) > limitExpendAmount) {
                            if (results.paymentControlCode === '01') {
