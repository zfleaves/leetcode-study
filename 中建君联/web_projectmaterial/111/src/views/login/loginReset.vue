<template>
  <div class='login-cons' :class="isMobileTerminal?'isMobileTerminal':''">
    ...
    <div class='login-tabs'>
      <div v-for='(item, index) in loginTabs' :key='index' :class="tabsKey === index ? 'active' : 'tabsList'"
        @click='loginTabsSubmit(index)'>{{item}}</div>
    </div>
    <!-- 密码登录 -->
    <div v-if='tabsKey === 0'>
      <el-form :model='loginForm' status-icon :rules='rules' ref='loginForm' class='demo-ruleForm'>
        <el-form-item prop='userCode'>
          <el-input v-model.number='loginForm.userCode' clearable :placeholder="$t('login.username')">
        ...
        <el-form-item prop='password' :error='loginForm.errorMsg'>
          <el-input type='password' v-model='loginForm.password' :placeholder="$t('login.password')"
            @keyup.enter.native="PsssubmitForm('loginForm')" clearable auto-complete='off'>
            <i slot='prefix' class='el-input__icon el-icon-unlock midFont grey'></i>
          </el-input>
        </el-form-item>
      </el-form>
      <el-button type='primary' size='small' :disabled='isDisabled' @click.native.prevent="submitForm('loginForm')"
    ...
    <div v-if='tabsKey === 1'>
      <el-form :model='loginMsg' status-icon :rules='rules' ref='loginMsg' class='login-msg-form'>
        ...
        <el-form-item prop='code' :error='loginMsg.errorMsg'>
          <el-col :span='24'>
            <el-input v-model.trim='loginMsg.code' maxlength='6' placeholder='短信验证码'>
      ...
      <el-button :disabled='isDisabled1' type='primary' size='small' @click.native.prevent="submitFormMsg('loginMsg')"
        :loading='logining'>登录</el-button>
    </div>
    <p class='midFont' style='margin-top: 20px'>
      <el-checkbox v-model='remember' style='float: left;' @click.prevent='rememberPassword($event)'>
