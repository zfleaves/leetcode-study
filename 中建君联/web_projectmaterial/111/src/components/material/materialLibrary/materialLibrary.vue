 ...
 * @LastEditTime: 2020-09-08 11:52:43
      ...
      <div class="left" v-loading="loading" :element-loading-text="$t('tips.loadingText')">
        <div class="title">
          ...
          <el-input v-model.trim="materialName" prefix-icon="el-icon-search" :placeholder="$t('tips.varietiesNameTips')"
            size="small" clearable @keyup.enter.native="_setTableDataList('search')" maxlength="64"></el-input>
        </div>
        <g-query-table :config="mainTable" @tableList="getTableSeleList">
        </g-query-table>
        <div class="table_pages">
          <g-to-page ref="Page" :pageSize="pageSize" :total="total" @sentPages="getPages">
          </g-to-page>
        </div>
      </div>
      <div class="right">
...
export default {
    ...
    return {
      ...
      mainTable: {
        ...
        tableList: [
          ...
          // 单位
          { show: true, prop: 'unit', label: 'fConfig.unit', minWidth: '100' }
        ]
      },
      selectMainTable: {
        rowKey: 'materialId',
  ...
  methods: {
    _setTableDataList (type) {
      ...
      this.$store.dispatch('equipInstall/getBasematerialDetail', data).then(res => {
        if (res.status === 0) {
          res.results.records && res.results.records.forEach(item => {
            item.materialId = item.id;
            if (this.checkList.length) {
              const index = this.checkList.findIndex(v => v.materialId === item.materialId);
              if (index >= 0) {
                this.checkList.splice(index, 1);
              }
            }
          });
          !type && (this.mainTable.selectedList = this.selectList);
    ...
    getTableSeleList (val) {
      ...
      for (const item of data) {
        const index = this.selectDialogList.findIndex(val => val.materialId === item.materialId);
