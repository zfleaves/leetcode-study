    ...
    return {
      ...
      mainTable: {
        ...
        tableList: [
          ...
          // 数量
          { show: true, prop: 'currentIncomingQuantity', label: 'fConfig.quantity', minWidth: '60' }
        ]
      },
      selectMainTable: {
        rowKey: 'eId',
    ...
    _setTableDataList (type) {
      ...
      this.$store.dispatch(dispatchApi, data).then(res => {
        if (res.status === 0) {
          const tableData = [];
          if (this.isIncoming) {
            for (const sele of this.selectListIncoming) {
                ...
                if (sele.incomingId === item.id) {
                  ...
                  item.incomingDetails.forEach(val => {
                    ...
                    if (this.checkList.length) {
                      const index = this.checkList.findIndex(v => v.eId === val.eId);
                      if (index >= 0) {
            ...
            for (const sele of this.selectListReturn) {
              for (const item of res.results) {
          ...
          if (type === 'search') {
            this.mainTable.tableData = tableData;
          } else {
            this.mainTable.tableData = tableData;
