  ...
  export default {
    ...
    methods: {
      _getTableDataList () {
            ...
            this.$store.dispatch('publicApi/getMasterPlanByprojectPublicApi', data).then(res => {
                if (res.status === 0 && res.results && res.results.length) {
                  this.selectList = [];
                  const demMasterPlanGants = res.results[0].demMasterPlanGants;
                  this.setMasterPlanData(demMasterPlanGants, res.results[0].id);
                  console.log(demMasterPlanGants, 'demMasterPlanGants');
                  this.pageConfig.mainTable.tableData = demMasterPlanGants;
                } else {
                    if (res.results && res.results.length === 0) {
                        // 暂无数据！请前往项目进度云添加【主计划编制】
                        this.$message.error(this.$t('tips.getMasterPlanByprojectNull'));
                    } else {
      ...
      // 设置数据
      setMasterPlanData(data, progressMasterPlanId) {
          for (const i in data) {
                this.selectId === data[i].wbs && this.$set(data[i], 'checked', true);
                this.$set(data[i], 'taskName', data[i].text);
                this.$set(data[i], 'progressMasterPlanId', progressMasterPlanId);
                if (data[i].children && data[i].children.length) {
                    this.setMasterPlanData(data[i].children, progressMasterPlanId);
                }
          }
      },
      // 搜索
      ...
      filterNode(value, data) {
          if (!value.taskName) return true;
          return data.taskName.indexOf(value.taskName) !== -1;
      }
    }
  };
</script>
