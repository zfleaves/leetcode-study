 ...
 * @LastEditTime: 2022-02-28 14:16:01
...
<template>
    <div class="processApprovalPage">
            ...
            <g-edit-title :title="`${$t(`button.${parmasData.type}`)}`">
                <div v-if="isGetData">
                    <el-button type="primary"
                                icon="iconfont icon-submit" size="small"
                                v-fast-click
                                :disabled="processButStatus"
                                @click="nextStep">{{$t('button.submitNext')}}
                    </el-button>
                    <!-- 快速同意 -->
                    <el-button type="primary"
                                class="other" icon="el-icon-check" size="small"
                                v-fast-click
                                :disabled="processButStatus"
                                @click="quickConsent">{{$t('button.quickConsent')}}
                    </el-button>
                    <el-button type="primary"
                                class="urgent"
                                ...
                                v-fast-click
                                :disabled="processButStatus"
                                @click="TakeStepBack" v-if="isBeginNodeCode">{{$t('button.backStep')}}
                    </el-button>
                    <el-button type="primary"
                                class="other"
                                icon="el-icon-d-arrow-left" size="small"
                                v-fast-click
                                :disabled="processButStatus"
                                @click="Returnsponsor" v-if="isBeginNodeCode">{{$t('button.returnToSponsor')}}
                    </el-button>
                    <el-button type="primary"
                                class="more"
                                icon="el-icon-third-Monitor" size="small"
                                v-fast-click
                                :disabled="processButStatus"
                                @click="handleMonitor">{{$t('button.monitorProcess')}}
                    </el-button>
                    <el-button type="primary"
                                class="urgent" icon="el-icon-third-deleteMore" size="small"
                                v-fast-click
                                :disabled="processButStatus"
                                @click="handleDeleteProcess" v-if="!isBeginNodeCode">{{$t('button.deleteProcess')}}
                    </el-button>
                </div>
            </g-edit-title>
            <component
        ...
        <el-dialog  :title="`${$t(`menu.${parmasData.processPageName}`)} ${$t('tips.processSubmission')}`"
            v-if="dialogProcess"
            :top="processName==='nextProcess'?'10vh':'25vh'"
            :width="processName==='nextProcess'?'600px':'400px'"
            center
            :visible.sync="dialogProcess">
            <component
                @closeDialog="handleCloseDialog"
                @submitPorcess='submitPorcess'
                @nextProcess='submitNextProcess'
                :is="processName"
                :nextForm="nextProcessForm"
                :processSubmitApi="pageConfigEdit.processParmas"
...
export default {
  mixins: [process],
    ...
    components: {
        ...
        buildLogApply(resolve) {
            require(['views/buildLog/buildLogApply/buildLogApplyEdit.vue'], resolve);
        },
        // 其它日志
        buildLogOther(resolve) {
            require(['views/buildLog/buildLogOther/buildLogOtherEdit.vue'], resolve);
        },
        imageProgressWork(resolve) {
            require(['views/imageProgress/imageProgressWork/imageProgressWorkEdit.vue'], resolve);
        },
        imageProgressOther(resolve) {
            require(['views/imageProgress/imageProgressOther/imageProgressOtherEdit.vue'], resolve);
        },
        inspectRegister(resolve) {
            require(['views/inspect/inspectRegister/inspectRegisterEdit.vue'], resolve);
        },
         // 主计划
        demMasterPlan(resolve) {
            require(['views/demMaster/demMasterPlan/demMasterPlanEdit.vue'], resolve);
        },
        demMasterPlanAdjust(resolve) {
            require(['views/demMaster/demMasterPlanAdjust/demMasterPlanAdjustEdit.vue'], resolve);
        },
        // 月计划
        demMonthPlan(resolve) {
            require(['views/demMonth/demMonthPlan/demMonthPlanEdit.vue'], resolve);
        },
        demMonthPlanAdjust(resolve) {
            require(['views/demMonth/demMonthPlanAdjust/demMonthPlanAdjustEdit.vue'], resolve);
        },
        demMonthprogressReport(resolve) {
            require(['views/demMonth/demMonthprogressReport/demMonthprogressReportEdit.vue'], resolve);
        },
        // 周计划
        demWeekPlan(resolve) {
            require(['views/demWeek/demWeekPlan/demWeekPlanEdit.vue'], resolve);
        },
        demWeekprogressReport(resolve) {
            require(['views/demWeek/demWeekprogressReport/demWeekprogressReportEdit.vue'], resolve);
        },
        // 工期
        durationRiskRegister(resolve) {
            require(['views/durationRisk/durationRiskRegister/durationRiskRegisterEdit.vue'], resolve);
        },
        // 报告
        inspectReport(resolve) {
            require(['views/inspect/inspectReport/inspectReportEdit.vue'], resolve);
        }
    },
    methods: {
        // 提交流程
        ...
        judegMultitonColumns() {
            ...
            const singletonColumns = this.workflowFormColumnsList.singletonColumns;
            // 子表状态 multitonColumns
            const multitonColumns = this.workflowFormColumnsList.multitonColumns;
