 ...
 * @LastEditTime: 2022-03-03 15:26:41
...
<template>
    <div style="height: 100%;" class="weekResources">
        ...
        <div v-if="pageConfigLoading" class="tabCons" ref="tabCons">
            <div v-for="subTable in pageConfig.subTableConfig" :key="subTable.subTableName">
                    ...
                    <g-button :operationButtons="subTable.subTableButton" @editTableBut="mainOperateBtnSubTable">
                        <template slot-scope="{parameter, buttonConfig}" slot='button-sysHandleImport'>
                            <importFile :parameter='parameter' :buttonConfig='buttonConfig'
                                ...
                                ref='importAllIFile'
                                :subTable='subTable' :flowStatus="type === 'info'" :importUrl='importUrlSingleDetail'>
                            </importFile>
                        </template>
                    </g-button>
                </g-edit-title>
                <g-edit-pl-table
                    ...
                    <!-- 名称 -->
                    <template slot="edit-table-resourceName" slot-scope="{item, scope}">
        ...
        <g-dialog v-if="dialogVisible" :dialogConfig="dialogConfig" :isVisible.sync="dialogVisible">
             <!-- :varietiesDateilUrl='varietiesDateilUrl' -->
...
export default {
    ...
    data () {
        ...
        return {
            ...
            tableHeight: 400,
            otherTableData: [],
            importUrlSingleDetail: '/proprogress/plan/monthprogress/report/resource/detail/importExcel', // 甘特图工程量及资源信息明细导入
            that: '',
            tableList: [
    ...
    created () {
        if (this.page) {
            ...
            console.log(this.varietiesDateilUrl, 'varietiesDateilUrl');
    ...
    methods: {
        ...
        getData(arr) {
            ...
            for (const item of arr) {
                item.resourceName = item.resourceDetailName;
                item.alreadyInputQuantity = item.alreadyQuantity;
                item.alreadyInputQuantity = item.alreadyQuantity;
                data.push(item);
            }
        ...
        setSubTable(tableName, arr) {
            ...
            if (tableData.length) {
                ...
                for (const val of arr) {
                    if (item.masterPlanResourceDetailId === val.masterPlanResourceDetailId) {
                        selectTableList.push(item);
                    }
                }
                }
                for (const item of arr) {
                const index = tableData.findIndex(v => v.masterPlanResourceDetailId === item.masterPlanResourceDetailId);
                if (index < 0) {
                    selectTableList.push(item);
                }
                }
        ...
        handleCalculation({item, row, event, subTable, rowIndex}) {
            ...
            if (item.prop === 'includeTaxAmount' && (row.includeUnitPrice === '' || row.includeUnitPrice === null)) {
                ...
                this.$set(row, 'includeUnitPrice', !inputQuantity ? 0 : Number((includeUnitPrice || 0).toFixed(4)));
            }
        ...
        dialogEvent(event) {
            if (event === 'close') {
                ...
                for (const i in resourcesDetail) {
                    ...
                    item.repeatKey = `${item.resourceName}-${item.standards}-${item.unit}-${item.resourceTypeCode}`;
        ...
        getImportData(val) {
            for (const item of val.tableData) {
                const inputQuantity = Number(item.inputQuantity) || 0; // 数量
