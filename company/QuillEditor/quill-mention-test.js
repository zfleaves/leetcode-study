...
function attachDataValues(element, data, dataAttributes, tabItem = {}) {
  var mention = element
  const newData = {
    ...data,
    ...tabItem
  ...
  Object.keys(newData).forEach(function (key) {
    if (dataAttributes.indexOf(key) > -1) {
      mention.dataset[key] = newData[key]
    } else {
...
var Mention = /* #__PURE__ */(function () {
  function Mention(quill, options) {
    ...
    this.options = {
      source: null,
      renderItem: function renderItem(item, searchTerm, tabItem) {
        return ''.concat(item.value)
      },
      renderLoading: function renderLoading() {
        return null
      },
      onSelect: function onSelect(tabItem, item, insertItem) {
        insertItem(item)
      },
      isChangeLine: true, // 是否回车换行
      tabs: [],
    ...
    if (this.options.tabs.length > 1) {
      ...
      this.tabList.id = 'quill-mention-tab'
      this.mentionContainer.appendChild(this.tabList)
      // console.log('this.options: ', this.options)
      for (let i = 0; i < this.options.tabs.length; i++) {
        const item = this.options.tabs[i]
        var li = document.createElement('li')
        li.id = 'quill-mention-tab-item-' + i
        li.className = 'quill-mention-tab-item'
        li.dataset.index = i
        li.innerHTML = item.title
        li.onmouseup = this.onItemTabClick.bind(this)
        this.tabList.appendChild(attachDataValues(li, item, this.options.dataAttributes))
      }
      this.highlightTabItem()
      this.mentionCons = document.createElement('div')
      this.mentionCons.setAttribute('class', 'ql-mention-list-cons')
      console.log('this.mentionCons: ', this.mentionCons)
      this.mentionContainer.appendChild(this.mentionCons)
      ...
      this.mentionCons.appendChild(this.mentionList)
      this.mentionContainer.classList.add('hasTabs')
      this.scrollDom = this.mentionCons
    } else {
      this.hasTabs = false
      this.mentionList = document.createElement('ul')
      this.mentionList.id = 'quill-mention-list'
      ...
      this.scrollDom = this.mentionContainer
    }
  ...
  _createClass(Mention, [{
    ...
    value: function selectHandler() {
      ...
      // console.log(' this.options: ', this.options)
      return this.options.isChangeLine
    }
  }, {
    key: 'toggleTabItem',
    value: function toggleTabItem() {
      if (this.isOpen) {
        if (this.isOpen && !this.existingSourceExecutionToken) {
          ...
          this.itemIndex = 0
          if (this.mentionList.childNodes.length === 0) return
          this.highlightItem()
          return false
        }
    ...
    key: 'highlightTabItem',
    value: function onItemTabClick(e) {
      for (var i = 0; i < this.tabList.childNodes.length; i += 1) {
        this.tabList.childNodes[i].classList.remove('selected')
      }
      if (this.tabsIndex === -1 || this.tabList.childNodes[this.tabsIndex].dataset.disabled === 'true') {
        return
      }
      this.tabList.childNodes[this.tabsIndex].classList.add('selected')
    }
  }, {
    key: 'onItemTabClick',
    value: function onItemTabClick(e) {
      ...
      this.tabsIndex = e.currentTarget.dataset.index
      console.log('this.tabsIndex: ', this.tabsIndex)
      this.highlightTabItem()
      this.quill.focus()
      this.onSomethingChange()
    ...
    value: function highlightItem() {
      ...
      // console.log('this.mentionList: ', this.mentionList)
      if (this.itemIndex === -1) {
        return
      }
      ...
      if (scrollItemInView) {
        ...
        let containerTop = this.scrollDom.scrollTop
        let containerBottom = containerTop + this.scrollDom.offsetHeight
        if (itemPos < containerTop) {
          // Scroll up if the item is above the top of the container
        //   console.log('itemPos: ', itemPos)
          this.scrollDom.scrollTop = this.hasTabs ? itemPos - 34 : itemPos
        } else if (itemPos > containerBottom - itemHeight) {
          // scroll down if any part of the element is below the bottom of the container
        //   console.log('itemPos - containerBottom + itemHeight: ', itemPos - containerBottom + itemHeight)
          this.scrollDom.scrollTop += itemPos - containerBottom + itemHeight
        }
      }
    }
  }, {
    ...
    value: function selectItem() {
      ...
      const tabItem = this.options.tabs[this.tabsIndex]
      this.options.onSelect(tabItem, data, function (asyncData) {
    ...
    value: function insertItem(data, programmaticInsert) {
      ...
      // console.log('insertAtPos: ', insertAtPos)
      // console.log('options.blotName: ', options.blotName)
      // console.log('render: ', render)
      // console.log('Quill__default.sources.USER: ', Quill__default['default'].sources.USER)
    ...
    value: function renderNoData() {
      if (this.mentionContainer.getElementsByClassName('ql-mention-noData').length > 0) {
      ...
      loadingDiv.className = 'ql-mention-NoData'
      loadingDiv.innerHTML = '暂无数据'
    ...
    value: function renderList(mentionChar, data, searchTerm) {
      if (data && data.length > 0) {
        ...
        for (var i = 0; i < data.length; i += 1) {
          ...
          li.dataset.index = i
          li.innerHTML = this.options.renderItem(data[i], searchTerm, this.options.tabs[this.tabsIndex])
          ...
          this.mentionList.appendChild(attachDataValues(li, data[i], this.options.dataAttributes, this.options.tabs[this.tabsIndex]))
        }
    ...
    value: function containerBottomIsNotVisible(topPos, containerPos) {
      var mentionContainerBottom = topPos + this.scrollDom.offsetHeight + containerPos.top
    ...
    value: function containerRightIsNotVisible(leftPos, containerPos) {
      ...
      var rightPos = leftPos + this.scrollDom.offsetWidth + containerPos.left
    ...
    value: function setMentionContainerPosition_Normal() {
      ...
      var containerHeight = this.scrollDom.offsetHeight
      var topPos = this.options.offsetTop
      var leftPos = this.options.offsetLeft // handle horizontal positioning

      if (this.options.fixMentionsToQuill) {
        var rightPos = 0
        this.scrollDom.style.right = ''.concat(rightPos, 'px')
      } else {
        leftPos += mentionCharPos.left
      }

      if (this.containerRightIsNotVisible(leftPos, containerPos)) {
        var containerWidth = this.scrollDom.offsetWidth + this.options.offsetLeft
      ...
      this.scrollDom.style.top = ''.concat(topPos, 'px')
      this.scrollDom.style.left = ''.concat(leftPos, 'px')
    ...
    value: function onSomethingChange() {
      ...
      var mentionChar = _getMentionCharIndex.mentionChar
      var mentionCharIndex = _getMentionCharIndex.mentionCharIndex
      ...
      if (hasValidMentionCharIndex(mentionCharIndex, textBeforeCursor, this.options.isolateCharacter, textPrefix)) {
        ...
        if (textAfter.length >= this.options.minChars && hasValidChars(textAfter, this.getAllowedCharsRegex(mentionChar))) {
          ...
          this.options.source(_this5.options.tabs[_this5.tabsIndex], textAfter, function (data, searchTerm) {
