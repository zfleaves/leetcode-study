 ...
 * @LastEditTime: 2024-05-22 21:58:39
 * @FilePath: \leetcode-study\TypeScript\funciton.ts
 */
// 定义一个类  
class Animal {
    // 属性  
...
// 需要注意的是，尽管 TypeScript 提供了访问修饰符来确保类型安全并增强封装性，但这些修饰符在编译为 JavaScript 时会被移除，因为 JavaScript 不支持这些访问修饰符。因此，TypeScript 的访问修饰符主要用于在 TypeScript 编译时提供类型检查和错误提示，而不是在运行时提供真正的访问控制。在 JavaScript 中，你可以访问任何对象的任何属性（除非使用闭包、原型链或其他 JavaScript 特性来模拟私有性）。